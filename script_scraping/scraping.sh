##このscraping.shは、NCBIから興味のあるデータを一括ダウンロードしたり解凍したりするやつ
##scraping.shはディレクトリsamples/scriptに置いておくこと。パスが面倒になる
#!/bin/bash
...
##samples/samplelistのディレクトリに作った、新しいプロジェクト名を入れろ！！！
path=telluraves

##Accession.Rで作ったcsvファイルを入れろ！！列名はないほうがいい（Sansyou.csvを作成するときに、種名とアクセッションがズレるから）
csv=telluraves_sample.csv

##欲しいのは"refseq"（GCF_）？それとも"genbank"（GCA_）?
assembly=refseq

##まず、NCBIのIndexから興味ある生物種がずらっと並ぶディレクトリ(vertebrate_other)までのindex.htmlをダウンロード
wget -w 1 -nc ftp://ftp.ncbi.nlm.nih.gov/genomes/${assembly}/vertebrate_other/ -P ../samplelist/${path}/	#index.htmlの作成
...
do
	...
	wget -w 1 ftp://ftp.ncbi.nlm.nih.gov/genomes/${assembly}/vertebrate_other/$Latenname/all_assembly_versions/	-P ../samplelist/${path}/html_box/	#種それぞれのhtmlファイル

done < ../samplelist/${path}/${csv} ###PLEASE ENTER YOUR ACCESSION AND SPECIES NAME FILE
...
cat ../samplelist/$path/html_matome.txt | grep -e '<a href="ftp://ftp.ncbi.nlm.nih.gov:21/genomes/refseq/vertebrate_other/' | sed -e 's/<[^>]*>//g' > ../samplelist/$path/accession_all.txt
...
##ダウンロードしたいファイルの拡張子を-Aの後ろに指定
	wget -r -nd -w 1 -P ../samplelist/$path/data/$name/ ftp://ftp.ncbi.nlm.nih.gov/genomes/$assembly/vertebrate_other/$name/all_assembly_versions/$accession/${accession}_cds_from_genomic.fna.gz

##BUSCO用のprotein.faa
	wget -r -nd -w 1 -P ../samplelist/$path/data/$name/ ftp://ftp.ncbi.nlm.nih.gov/genomes/$assembly/vertebrate_other/$name/all_assembly_versions/$accession/${accession}_protein.faa.gz

##LSI extraction用のgffファイルも
	wget -r -nd -w 1 -P ../samplelist/$path/data/$name/ ftp://ftp.ncbi.nlm.nih.gov/genomes/$assembly/vertebrate_other/$name/all_assembly_versions/$accession/${accession}_genomic.gff.gz 
...
##解凍も忘れずに
	...
	gunzip ../samplelist/$path/data/${name}/${accession}_genomic.gff.gz

	gen=$(echo ${name:0:2})			#二文字の属名
	species=$(echo $name | cut -d "_" -f 2)
	spe=$(echo ${species:0:3})		#三文字の種小名
##例 gen.spe.faa   Pa.maj.faa
...
#	cp ../samplelist/$path/data/${name}/${gen}.${spe}.fna ../../transport/
	...
	mv ../samplelist/$path/data/${name}/${accession}_genomic.gff ../samplelist/$path/data/${name}/${gen}.${spe}.gff
#	cp ../samplelist/$path/data/${name}/${gen}.${spe}.gff ../../transport/
##ひとまず、CDSもproteinもgffも簡単な名前にして保存されているが、CDSとgffについてはLSIを行うので、注意
